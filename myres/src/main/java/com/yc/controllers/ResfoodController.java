...
public class ResfoodController {
    ...
    public JsonModel confirmOrder(HttpSession session, Resorder resorder, JsonModel jm) {
        ...
        Resuser resuser = (Resuser) session.getAttribute(ZhwConstans.LOGINUSER);
        resorder.setUserid(resuser.getUserid());
        //准备  Resorderitem数据
        if (session.getAttribute(ZhwConstans.CART) == null || ((Map<Integer, CartItem>) session.getAttribute(ZhwConstans.CART)).size() <= 0) {
            jm.setCode(0);
            jm.setMsg("you have not buy any thing..");
            return jm;
        }
        Map<Integer, CartItem> cart = (Map<Integer, CartItem>) session.getAttribute((ZhwConstans.CART));
        try {
